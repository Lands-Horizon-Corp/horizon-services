...
// Database defines lifecycle management and access to the underlying GORM client.
// It abstracts startup, shutdown, health checking, and raw client retrieval.
type Database interface {
	// Run starts or initializes any resources (connections, pools).
	Run(ctx context.Context) error

	// Stop cleanly shuts down and releases resources.
	Stop(ctx context.Context) error

	// Ping checks the database health (connectivity and responsiveness).
	Ping(ctx context.Context) error

	// Client returns the underlying *gorm.DB for advanced queries.
